...
| 가락시장 8초 피드 (선택) | data.go.kr/15004517 (서울시농수산식품공사) | 가락 단독 고해상도 (~8초) | ~8초 | 가락 단독 (위 전국의 부분집합) | ✅ 선택적 — 플래그십 시장 고해상도 원할 때만 |
...
**API 소스 (정형, 폴링) → `raw.recipes.api`**
| 소스 | 유형 | 합법성 |
|---|---|---|
| **식약처 COOKRCP01** (data.go.kr/15060073) | ~1,100건, 재료 자유텍스트 | ✅ 합법 |
| **농교원 레시피 재료정보** (data.go.kr/15058981) | 재료명+수량+단위 structured | ✅ 합법 (NER gold label) |
| **농교원 레시피 기본정보** (data.go.kr/15057205) | 요리명/시간/난이도/분류 | ✅ 합법 (실존 확인) |
| ~~농진청 메뉴젠 (15081047)~~ | ~~음식+재료+조리~~ | ❌ 404(잘못된 ID) — 드롭 |
...
| 가락 8초 피드 (선택, data.go.kr/15004517) | 공공 | ~8초 | 플래그십 시장 고해상도 (원할 때만) |
> 도매 경락가 = 내일 소매가 선행지표 → "지금 사? 기다려?" 시세추천 + **멀티마켓 비교("가락 vs 구리 vs 강서 어디가 싼가")** + 예산 밀플래닝 핵심.
> **32개 시장 × 장중 연속 = 진짜 대량 스트림** → Kafka 파티셔닝(시장별/품목별) 설계 명분. 가락 단독은 이 전국 피드의 부분집합.
> ⚠️ 단서: "실시간"=각 시장 경매시간(새벽~오전) 중 계속 게시. 전국 일괄 8초틱은 아니나 일별 배치보다 압도적. 표준데이터(15029181)는 일별집계라 혼동 금지. production 티어 승급은 use-case 등록 필요.
...
*⑤ 고빈도 API 폴 (스트림급 아님, 스케줄 배치) → `raw.*.api`*
| 소스 | 갱신 | 얻는 것 |
|---|---|---|
| **NEIS 학교급식 식단** (data.go.kr/15139198) | 일별 ~11,000교 | 대량 메뉴+재료+알레르기+원산지 (NER 처리 대상) |

**폴백 (가락시장 API가 장중집계로 판명 시):** 위키피디아 EventStreams (CC-BY-SA, 연속 편집 크롤)로 연속 스트림 명분 보강
...
> - **실시간 스트림** = 가락시장 경락가(가격축). 학습 아니라 런타임 시세.
> - **크롤(런타임 콘텐츠)** = 향토음식 벌크 + NEIS. 학습된 NER이 처리할 입력 공급 (load-bearing).
...
| 일별 도·소매가격(15156057, aT) | 자동승인 | 조사일자/품목명/**구분코드(도·소매 구분)**/시군구/시장/조사일가격/kg환산가격 |
...
| 소스 | 확인할 것 |
|---|---|
| 소비자원 참가격(3043385) | **판매가격·판매업소(마트명)** → P0 #9 마트비교 사활 |
| 농교원 재료정보(15058981) | **분량·단위 분리 필드** → NER gold label 근거 |
| 온라인가격(15080757) | 가격 operation 실제 필드 |

**❌ 깨짐 — fork 오류 검출**
| 소스 | 문제 |
|---|---|
| 농진청 메뉴젠(15081047) | **404 페이지 없음. fork가 잘못된 ID 제공.** 레시피는 COOKRCP01+농교원 2종으로 충분 → 메뉴젠 드롭(정확 ID는 추후 재확인) |

**다음 단계:** data.go.kr 서비스키 1개 발급(1키=활용신청 모든 API 공통, 다수 자동승인=즉시) → 리스크 3건(참가격 마트명·가격 / 재료정보 분량·단위 / 온라인가격) + hero(15141808) 실 응답 필드 확인 → 확정 후 설계 락.
...
- 학습 데이터: 농교원 재료정보 API(data.go.kr/15058981)에 **재료명+수량+단위 structured gold label** 존재 + 식약처 COOKRCP01(~1,100건) 자유텍스트
- **모델: CRF(Conditional Random Field) 기반 NER — CPU 학습+서빙** (KoBERT 대신; 3GB VRAM 제약). structured gold label 덕에 CRF도 충분한 정확도
...
가락 8초 피드(선택)          →  raw.prices.garak.hires   →  고해상도 시세 (플래그십)
...
NEIS 급식 폴러(일별)          →  raw.recipes.neis         →  재료 NER 추출기 → recipes DB
...
```
- 재료 NER 추출기 = `crawled.bulk` + `neis` + `user.ocr` **3개 컨슈머** → NER이 파이프라인 중심
- 갱신주기: **실시간 8초(가락 경락가)** / 시간별(기상) / 일별(NEIS/KAMIS) / 이벤트(회수) / 격주(참가격) / 분기(향토음식) = **극단적 이기종 스케줄 = K8s test #1 최강 + 진짜 스트림 확보**
...
| 2026-07-08 | **데이터소스 실검증 실시(§2.5): 식약처·NEIS 라이브 확인 / 8개 실존확인 / 메뉴젠(15081047) 404 검출·드롭** | fork 보고 맹신 리스크 실제 검출. 리스크 3건(참가격 마트명·가격/재료정보 분량단위/온라인가격)은 키 발급 후 확인 |
