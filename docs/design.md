...
| ~~참가격 (3043385)~~ | ~~격주~~ | ❌ **DEAD** (API 폐기, §2.5 참조) |
...
### Kafka 정당화 (억지가 아닌 이유)
- **이기종 프로듀서**: 전국시장 실시간경매(장중 스트림), KAMIS 폴러(일별), 식품회수(이벤트), KOGL 크롤러(벌크), 유저 OCR(실시간) — 각각 다른 주기/형식
...
~~참가격 폴러(격주)            →  raw.prices.chamgagyeok   →  가격 정규화기 → prices DB~~ ❌ DEAD
...
- 갱신주기: **실시간(전국시장 경락가)** / 시간별(기상) / 일별(KAMIS) / 이벤트(회수) / 분기(향토음식) = **이기종 스케줄 = K8s test #1 + 진짜 스트림**
...
| **Price Poller** | KAMIS(일별) + ~~참가격(격주, DEAD)~~ + 국가데이터처(일별, P1) → Kafka | 소스별 상이 | CronJob |
...
| **ML Training Job** | 4종 모델 재학습 (CRF NER, LightGBM 시세예측, XGBoost 신선도, LightGBM 랭킹) | 주간/월간 | Job (**CPU**) |
...
ClickHouse (1대, Altinity operator)
  └── 가격 시계열·시세예측 피처  ← Price Service / ML Training
...
| **3. 반복 재학습** | ✅ 통과 | 새 레시피 → NER 재학습, 실시간 경락가 유입 → 시세예측 모델, 유저 피드백 → 신선도 모델. 데이터 drift가 진짜임 (인위적 아님) |
...
- 스파이크(#4) 시연 = **Karpenter**로 워커 노드 확장 (EC2 직접 프로비저닝, 순수 AWS 오토스케일)
